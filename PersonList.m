     ...
     sqlite3_stmt *sqlStatement;
        @try {
            ...
            while(sqlite3_step(sqlStatement) == SQLITE_ROW) {
                ...
                personDAO.personID = sqlite3_column_int(sqlStatement, 0);
...
-(PersonDAO *) getPerson:(NSInteger)personID {
    ...
    PersonDAO *personReturn = [[PersonDAO alloc] init];
    sqlite3_stmt *statement;
        ...
        if(sqlite3_open(dbPath, &db) == SQLITE_OK) {
            ...
            if(sqlite3_prepare_v2(db, queryStatement, -1, &statement, NULL) == SQLITE_OK) {
                if(sqlite3_step(statement) == SQLITE_ROW) {
                    personReturn.firstName = [NSString stringWithUTF8String:(char *) sqlite3_column_text(statement, 0)];
                    personReturn.lastName = [NSString stringWithUTF8String:(char *) sqlite3_column_text(statement, 1)];
                    personReturn.telephone = [NSString stringWithUTF8String:(char *) sqlite3_column_text(statement, 2)];
                }
            }
        }
    }
